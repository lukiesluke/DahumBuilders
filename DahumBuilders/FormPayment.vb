...
Public Class FormPayment
    ...
    Public Sub load_userId_info_data_reader()
        ...
        IFNULL((SELECT i.`price`-(SUM(`paid_amount`)-(SUM(`discount_amount`)+SUM(`penalty`))) FROM `db_transaction` WHERE db_transaction.`proj_id`=i.`proj_id` AND db_transaction.`proj_itemId`=i.`item_id` AND i.`assigned_userid`=db_transaction.`userid`), i.`price`) AS 'totalBalance',
